...
#ifdef HAL_UART_485
  /* EP»òROUTER 485Ä£Ê½ÅäÖÃÎª½ÓÊÕ */
  P0SEL             &= ~0x10;         //P0.4ÖØ¶¨ÒåÎªGPIO
  P0DIR             |= 0x10;          //P0.4¶¨ÒåÎªÊä³ö¿Ú
  RS485_WR_PORT     = 0;              //³õÊ¼»¯Îª½ÓÊÕÄ£Ê½  
#endif  
 ...
 * @fn      HalUARTWrite2
 *
 * @brief   Write a buffer to the UART, using direct io pins.
 *
 * @param   port - UART port
 *          buf  - pointer to the buffer that will be written, not freed
 *          len  - length of
 ...
 * @return  length of the buffer that was sent
 *****************************************************************************/
uint16 HalUARTWrite2 (uint8 port, uint8 *buf, uint16 len)
{
    uint16 i = 0;
