...
u8 Driver_AT24CXXProbe(void)
{
	...
	{
	    UCORE_DEBUG_PRINT("Probe EEPROM AT24C128 success!\r\n");
...
u8 AT24CXX_ReadBytes(u8 *_pReadBuf, u16 _usAddress, u16 _usSize)
{
...
#if 1
	i2c_SendByte(_usAddress >> 8);  /* send HI address */
    if (i2c_WaitAck() != 0)
	{
		goto cmd_fail;	/* EEPROM器件无应答 */
	}
    ...
    i2c_SendByte(_usAddress);  /* send LOW address */    
    if (i2c_WaitAck() != 0)
	{
		goto cmd_fail;	/* EEPROM器件无应答 */
	}
	...
	for (i = 0; i < _usSize; i++)
	{
		...
		if ((i == 0) || (usAddr & (EE_PAGE_SIZE - 1)) == 0)
		{
			...
			*/
			...
			/* 第4步：发送字节地址，24C02只有256字节，因此1个字节就够了，如果是24C04以上，那么此处需要连发多个地址 */
#if 1
	        i2c_SendByte(usAddr >> 8);  /* send HI address */            			
			if (i2c_WaitAck() != 0)
			{
				goto cmd_fail;	/* EEPROM器件无应答 */
			}
            ...
            i2c_SendByte(usAddr);  /* send LOW address */
			if (i2c_WaitAck() != 0)
			{
				goto cmd_fail;	/* EEPROM器件无应答 */
			}
