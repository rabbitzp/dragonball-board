...
/*-- GPIO Configuration ------------------------------------------------------*/
/* D0->D3, NOE, NWE and NCE2  NAND pin configuration  */
...
void FSMC_NAND_ReadID(NAND_IDTypeDef* NAND_ID)
{
  ...
  *(vu8 *)(Bank_NAND_ADDR | ADDR_AREA) = 0x00;
   ...
   printf("nand id maker 0x%02x, device 0x%02x, third 0x%02x, fourth 0x%02x\n",
...
#define db_NAND_SIZE	0x4000000	// 64MB 0x76 0x20
#define db_NAND_PAGES	(db_NAND_SIZE/NAND_PAGE_SIZE)	// 64MB 0x76 0x20


u32 db_nand_erase_page(u32 page_addr)
{
  *(vu8 *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_ERASE0;

  *(vu8 *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_1st_CYCLE(page_addr);
  *(vu8 *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_2nd_CYCLE(page_addr);
  *(vu8 *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_3rd_CYCLE(page_addr);
...
u32 db_nand_write_page(u8 *pBuffer, u32 page_addr)
{
  u32 index = 0x00, i;
  u32 status = NAND_READY;

  if (page_addr >= db_NAND_PAGES)
  	return 1;

  if (!(page_addr % NAND_BLOCK_SIZE))
        db_nand_erase_page(page_addr);
    ...
    *(vu8 *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_3rd_CYCLE(page_addr);
    ...
    {
      *(vu8 *)(Bank_NAND_ADDR) = pBuffer[index];
    }
...
u32 db_nand_read_page(u8 *pBuffer, u32 page_addr)
{
  u32 index = 0x00;
  u32 status = NAND_READY;
  int i=0;
    ...
    *(vu8 *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_AREA_TRUE1; 
    ...
    for(index = 0; index < NAND_PAGE_SIZE; index++)
    {
      pBuffer[index]= *(vu8 *)(Bank_NAND_ADDR);
    }
...
static u8 testw[NAND_PAGE_SIZE] = {0};
static u8 testr[NAND_PAGE_SIZE] = {0};

u32 db_nand_test_page(u32 page)
{
	static vu8 fill = 0;
	u32 i = 0;
	static vu32 time_b = 0, time_b1 = 0, time_b2 = 0, time_n = 0;
	
	for (i = 0; i < NAND_PAGE_SIZE; i++)
		testw[i] = fill++;

	time_b = OSTimeGet();
	if (db_nand_write_page(testw,page) != NAND_READY)
	{
		printf("page %d write err\n", page);
		return 1;
	}

	time_b1 = OSTimeGet();
	if (db_nand_read_page(testr,page) != NAND_READY)
	{
		printf("page %d read err\n", page);
		return 2;
	}
	...
	for (i = 0; i < NAND_PAGE_SIZE; i++)
		if (testw[i]!=testr[i])
		{
			printf("page %d at %d err, w 0x%02x, r 0x%02x\n",
				page, i, testw[i], testr[i]);
			return 3;
		}
	time_n = OSTimeGet();
	
	printf("test nand page %d OK, w %d, r %d, all %d ms\n", 
		page, time_b1-time_b, time_b2-time_b1, time_n-time_b);
...
void db_nand_test(void)
{
	u32 i = 0;
	for (i = 0; i < db_NAND_PAGES; i++)
		db_nand_test_page(i);
	return;
}
void db_nand_init(void)
{
    /* Bus clock init */
...
void NAND_TaskProc(void *p_arg)
{
	printf("User Task nand Start.\r\n");
	...
	printf("User Task nand Exit.\r\n");
}
OS_EVENT            *g_QMboxNANDMsg = NULL;
#define DEFAULT_USER_TASK_NAND_PRIO	12
#define DEFAULT_USER_NAND_STK_SIZE	64

static OS_STK       g_UserTaskNANDSTK[DEFAULT_USER_NAND_STK_SIZE];
...
u8 NAND_Start(void)
{
    ...
    printf("starting user task nand...\r\n");

    ucResult = OSTaskCreate(NAND_TaskProc, (void *)0, (OS_STK *)&g_UserTaskNANDSTK[DEFAULT_USER_NAND_STK_SIZE - 1], OS_LOWEST_PRIO-5);
    ...
    printf("User task nand Failed value:%d.\r\n", ucResult);
